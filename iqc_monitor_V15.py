...
def calculate_workload_metrics(processed_df, additional_tasks_df, start_date=None, end_date=None):
    ...
    if not workload_df.empty:
        # 向量化計算總時間和工作負載指數
        workload_df['total_time'] = workload_df['inspection_time'] + workload_df['additional_task_time']
...
def calculate_all_metrics(iqc_df, pcb_specs_df, pcb_standard_time_df, additional_tasks_df, start_date=None, end_date=None):
    try:
        ...
        workload_data = calculate_workload_metrics(iqc_data_with_pcb_time, additional_tasks_df, start_date, end_date)
        workload_end = time.time()
        debug_log(f"工作負載指標計算用時: {workload_end - workload_start:.2f}秒", level="INFO")
        
        # 計算時間分配和額外任務指標
        time_allocation_start = time.time()
        time_allocation_data = calculate_time_allocation_metrics(iqc_data_with_pcb_time, additional_tasks_df, start_date, end_date)
        ...
        return {
            'processed_data': iqc_data_with_pcb_time,
...
def render_efficiency_dashboard(efficiency_data, processed_data=None):
    ...
    # 設置標題和軸標籤
    fig.update_layout(
        title="IQC人員總檢驗效率排名" + (f" (剔除極值 {st.session_state.get('trim_percentage', 0)}%)" if st.session_state.get('trim_percentage', 0) > 0 else ""),
        xaxis_title="檢驗員",
        yaxis_title="檢驗效率",
        height=500
    )
...
def render_workload_dashboard(workload_data):
    ...
    # 按檢驗員分組
    inspectors = workload_data['inspector'].unique()
    
    # 繪製工作負載指數趨勢圖
    fig = go.Figure()
    
    for inspector in inspectors:
        df_inspector = workload_data[workload_data['inspector'] == inspector]
        
        fig.add_trace(go.Scatter(
            x=df_inspector['date'],
            y=df_inspector['workload_index'],
            mode='lines+markers',
            name=inspector,
            hovertemplate='%{y:.2f} (檢驗次數: %{text})',
            text=df_inspector['inspection_count']
        ))
    ...
    fig.update_layout(
        title="IQC人員工作負載指數趨勢",
        xaxis_title="日期",
        yaxis_title="工作負載指數 (工作時間/8小時)",
        height=600,
        # 改為水平圖例並放在圖表底部
        legend=dict(
            orientation="h",      # 水平排列
            y=-0.2,               # 移到圖表下方 (-0.2 表示在圖表下方20%的位置)
            x=0.5,                # 水平居中
            xanchor="center",     # 水平居中對齊
            yanchor="top",        # 頂部對齊
            font=dict(size=10),   # 減小字體大小
            itemsizing="constant" # 統一圖例項目大小
        ),
        hovermode="x unified",
        margin=dict(b=150)  # 增加底部邊距，為圖例留出足夠空間
    )
    
    # 添加參考線 (工作負載為1的水平線)
    fig.add_shape(
        type="line",
        x0=workload_data['date'].min(),
        y0=1,
        x1=workload_data['date'].max(),
        y1=1,
    ...
    fig_avg = px.bar(
        ...
        color_continuous_scale=[
            [0, "rgb(0, 0, 180)"],     # 深藍色 - 低
            [0.2, "rgb(0, 120, 255)"],  # 淺藍色 - 正常
            [0.4, "rgb(0, 200, 0)"],   # 綠色 - 標準
            [0.7, "rgb(255, 165, 0)"],  # 橘黃色 - 高
            [1.0, "rgb(255, 0, 0)"]     # 紅色 - 嚴重
        ],
        range_color=[0, max_workload]
    )
    ...
    # 檢驗次數分析
    if 'inspection_count' in workload_data.columns:
        st.subheader("檢驗次數分析🔢 ")
    ...
    with st.expander("查看工作負載詳細資料"):
        ...
        display_df['inspection_time'] = display_df['inspection_time'].round(2)
...
def main():
    ...
    # 添加標題和描述
    st.title("IQC 效率管理系統")
    st.markdown("透過數據量化分析，分析IQC的檢驗效率、工作負載、與時間管理分配，從而協助提升IQC效能與品質控制水平。")
    # 建立側邊欄
    create_sidebar()
    ...
    if st.session_state.files_uploaded:
        ...
        button[data-baseweb="tab"] {
            ...
            color: black !important;  /* 未選中的標籤頁為黑色文字 */
        ...
        button[data-baseweb="tab"][aria-selected="true"] {
            ...
            color: white !important;  /* 選中的標籤頁為白色文字 */
        ...
        [data-testid="stTabs"] [data-baseweb="tab"] * {
            color: inherit !important;  /* 繼承父元素的顏色 */
        }
        </style>
        """, unsafe_allow_html=True)
